...
test("application migrations are globbed while the auth source remains outside", () => {
  const migrationsDir = join(projectRoot(), "migrations");
  assert.deepEqual(pendingMigrations(readdirSync(migrationsDir), []), [
    { name: "0002_leads.sql", path: "0002_leads.sql" },
    { name: "0003_lead_consent.sql", path: "0003_lead_consent.sql" },
  ]);
